            ...
            while (true) {
                ...
                if (obj2 == SelectKt.UNDECIDED) {
                    ...
                    if (Result.m40isFailureimpl(obj)) {
                        Continuation<R> continuation = this.uCont;
                        Throwable r4 = Result.m37exceptionOrNullimpl(obj);
                        Intrinsics.checkNotNull(r4);
                        Result.Companion companion = Result.Companion;
                        if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
                            r4 = StackTraceRecoveryKt.recoverFromStackFrame(r4, (CoroutineStackFrame) continuation);
                        }
                        continuation.resumeWith(Result.m34constructorimpl(ResultKt.createFailure(r4)));
                        return;
                    }
    ...
    public void resumeSelectWithException(Throwable th) {
        if (!DebugKt.getASSERTIONS_ENABLED() || isSelected()) {
            while (true) {
                ...
                if (obj == SelectKt.UNDECIDED) {
                    Continuation<R> continuation = this.uCont;
                    if (_result$FU.compareAndSet(this, SelectKt.UNDECIDED, new CompletedExceptionally((!DebugKt.getRECOVER_STACK_TRACES() || !(continuation instanceof CoroutineStackFrame)) ? th : StackTraceRecoveryKt.recoverFromStackFrame(th, (CoroutineStackFrame) continuation), false, 2, null))) {
                        return;
                    }
                } else if (obj != IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
                    throw new IllegalStateException("Already resumed");
